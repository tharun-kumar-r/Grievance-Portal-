...
function sendmail($mai,$gripro,$umail,$subjct,$clgnam)
{
  ...
  $mai = new PHPMailer;
            $mai->isSMTP();
            $mai->Debugoutput = 'html';
            $mai->Host = 'smtp.gmail.com';
            $mai->Port = 587;
            $mai->SMTPSecure = 'tls';
            $mai->SMTPAuth = true;
            $mai->Username = 'serversmptmailrajsoft@gmail.com';
            $mai->Password = 'x.html.php';
            $mai->setFrom('noreplay@mail.com', $clgnam);
            $mai->addAddress($umail, 'ADMIN@Grievance Redressal Cell');
            
            $mai->Subject = 'Admin@Grievance Redressal Cell';
            $mai->isHTML(true);
            $mai->Body = '<body bgcolor="silver" style="font-family:arial"><center><br>
